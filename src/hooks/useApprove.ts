...
import { useTokenAllowance } from "../state/tokenLists/hooks";
...
export function useApproveCallback(
  ...
  const approve = useCallback(async (): Promise<void> => {
    ...
    setApproved(false);
  }, [approveMaxCallback, token, spender]);
...
//   spender?: string,
//   amountsToApprove?: Array<string | number>
// ): {
...
//       if (!token || !token.address || !spender) {
//         return ApprovalState.UNKNOWN;
//       }
//       if (token.address === EtherKey) {
//         return ApprovalState.APPROVED;
//       }
...
//         for (let i = 0; i < needApprovalTokens.length; i++) {
//           const token = needApprovalTokens[i];
//           if (!token?.address) {
//             console.error('no token');
//           }
//           if (!spender) {
...
//             const latest = i === needApprovalTokens.length - 1;
//             if (latest) {
//               await tokenStore.approveMaxCallback(token.address, spender);
//             } else {
//               await tokenStore.approveMax(token.address, spender);
//             }
//           }
//         }
//       } catch (e) {}
//       setApproved(false);
//     }
//   }, [tokenStore, spender, approvalState]);
